...
always@(posedge CLK,posedge RST)
    begin
        if(RST) TX_SAMP_CT <= 4'b0000;
        else if(TXCT_R) TX_SAMP_CT <= 4'b0000;
        else if(UART_CE) TX_SAMP_CT <= TX_SAMP_CT + 1'b1;
    end
assign TX_CE = UART_CE & TX_SAMP_CT[3] & TX_SAMP_CT[2] & TX_SAMP_CT[1] & TX_SAMP_CT[0]; // if CE and TX_SAMP_CT == 4'b0111 (4'd15)
